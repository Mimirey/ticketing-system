...
import { getTicket, assignTicket, updateTicketStatus, updateTicketPriority } from "../api/ticket";
...
export default function TicketDetail() {
  ...
  if (!ticket) return <div className="p-6 text-slate-400">Ticket tidak ditemukan</div>;
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-xs text-slate-400 mb-1">{ticket.ticket_number}</p>
        ...
        <p className="text-sm text-slate-600 mb-4 whitespace-pre-wrap">{ticket.description}</p>

        <div className="grid grid-cols-2 gap-3 text-sm text-slate-500 mb-4">
          <span>Jenis: <span className="text-slate-700">{ticket.type}</span></span>
          <span>Modul: <span className="text-slate-700">{ticket.module || "-"}</span></span>
        </div>

        {actionError && (
          <div className="mb-4 p-3 bg-red-50 text-red-600 text-sm rounded">{actionError}</div>
        )}

        <div className="border-t border-slate-100 pt-4 flex flex-wrap gap-4">
          {isPM && ticket.status !== "Done" && (
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Assign ke Staff IT</label>
              <select
                value={ticket.pic_id ?? ""}
          ...
          {isPM && ticket.status !== "Done" && (
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Ubah Prioritas</label>
              <select
          ...
          {canChangeStatus && nextStatuses.length > 0 && (
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Ubah Status</label>
              <select
                value=""
                onChange={(e) => e.target.value && handleStatusChange(e.target.value)}
                className="px-2 py-1.5 border border-slate-300 rounded-md text-sm"
              >
                <option value="" disabled>Pilih status berikutnya</option>
                {nextStatuses.map((s) => (
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200">
        <div className="flex border-b border-slate-200">
          {(["comments", "attachments", "history"] as Tab[]).map((t) => (
            ...
            >
              {t === "comments" ? "Komentar" : t === "attachments" ? "Lampiran" : "Riwayat"}
            </button>
          ))}
        </div>
